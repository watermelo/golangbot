...
在上面的程序中的第 8 行，我们为变量`a`分配了一个函数。 这是将函数赋值给变量的语法。如果你仔细观察的话会发现，分配给`a`的函数没有名称。这些没有名称的函数称为匿名函数。
...
闭包是匿名函数的特例。闭包是匿名函数，用于访问该匿名函数体外定义的变量。
...
每个闭包都把变量绑定到它周围的变量。让我们通过一个简单的例子来理解这是什么意思。
...
我们先用参数`World`调用`a`。现在，`a`闭包`t`的值变成了`Hello World`。

在第 20 行，我们用参数`Everyone`调用`b`。由于`b`绑定到自己的变量`t`，因此`b`闭包的初始值为`Hello`。在此函数调用之后，`b`闭包变量`t`的值将变为`Hello Everyone`。
...
到目前为止，我们已经定义了头等函数，并且我们已经编写了一些例子来了解它们的工作原理。现在让我们编写一个具体的程序，它揭示了头等函数的实际用法。
...
下一步是编写`filter`函数。该函数的入参为`student`结构的切片和一个确认`student`是否符合条件的函数。编写这个函数，能让我们更好地理解。让我们继续吧。
...
在上面的函数中，第二个参数是一个以`student`为参数并返回`bool`的函数。此函数是为了确认该`student`是否符合条件。我们在第 3 行中迭代`student`切片，然后将每个`student`作为参数传递给函数`f`。如果返回`true`，则表示`student`符合过滤条件，并将其添加到结果切片`r`中。你可能对此功能的实际使用感到有些困惑，继续完成该程序就会更清楚。我添加了 main 函数，并在下面提供了完整的程序。
...
让我们再写一个程序来结束本节。该程序将对切片的每个元素执行相同的操作并返回结果。例如，如果我们想要将切片中的所有整数乘以 5 并返回输出，则可以使用头等函数轻松完成。这些对集合的每个元素进行操作的函数称为`map`函数（译者注：map/reduce 函数）。我提供了以下程序。
